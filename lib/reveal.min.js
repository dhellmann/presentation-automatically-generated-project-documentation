...
(function( root, factory ) {
	...
	function start() {
		...
		setTimeout( function() {
			...
			dispatchEvent( dom.wrapper, 'ready', {
				'indexh': indexh,
				'indexv': indexv,
	...
	function setupPDF() {
		...
		dispatchEvent( dom.wrapper, 'pdf-ready' );
	...
	function configure( options ) {
		...
		var transitionBefore = config.transition;
		var fragmentsBefore = config.fragments;
		...
		dom.wrapper.classList.remove( transitionBefore );
		...
		// If fragments are disabled, show them
		toArray( dom.slides.querySelectorAll( '.fragment' ) ).forEach( function( element ) {
			if( config.fragments === false ) {
				element.classList.add( 'visible' );
			}
			else if( fragmentsBefore === false ) {
	 ...
	 * reveal DOM element.
	 */
	function dispatchEvent( element, type, properties, options ) {

		options = extend( { bubbles: true }, options );

		var event = document.createEvent( 'HTMLEvents', 1, 2 );
		event.initEvent( type, options.bubbles, true );
	...
	function layout() {
		...
		if( dom.wrapper && !isPrintingPDF() ) {
			...
			if( !config.disableLayout ) {
				...
				if( oldScale !== scale ) {
					dispatchEvent( dom.wrapper, 'resize', {
						'oldScale': oldScale,
						'scale': scale,
						'size': size
					}, { bubbles: false } );
	...
	function activateOverview() {
		...
		if( config.overview && !isOverview() ) {
			...
			dispatchEvent( dom.wrapper, 'overviewshown', {
				'indexh': indexh,
				'indexv': indexv,
				'currentSlide': currentSlide
			} );
	...
	function deactivateOverview() {
		...
		if( config.overview ) {
			...
			dispatchEvent( dom.wrapper, 'overviewhidden', {
				'indexh': indexh,
				'indexv': indexv,
				'currentSlide': currentSlide
			} );
	...
	function pause() {
		...
		if( config.pause ) {
			...
			if( wasPaused === false ) {
				dispatchEvent( dom.wrapper, 'paused' );
			}
		}
	...
	function resume() {
		...
		if( wasPaused ) {
			dispatchEvent( dom.wrapper, 'resumed' );
		}
	...
	function slide( h, v, f, o ) {
		...
		stateLoop: for( var i = 0, len = state.length; i < len; i++ ) {
			...
			dispatchEvent( dom.wrapper, state[i] );
		}
		...
		if( slideChanged ) {
			dispatchEvent( dom.wrapper, 'slidechanged', {
				'indexh': indexh,
				'indexv': indexv,
	...
	function updateSlides( selector, index ) {
		...
		if( slidesLength ) {
			...
			var wasPresent = slide.classList.contains( 'present' );
			...
			if( !wasPresent ) {
				...
				dispatchEvent( slide, 'visible', null, { bubbles: false } );
			}

			// If this slide has a state associated with it, add it
			// onto the current state of the deck
			var slideState = slide.getAttribute( 'data-state' );
			if( slideState ) {
	...
	function autoAnimate( fromSlide, toSlide ) {
		...
		if( fromSlide.hasAttribute( 'data-auto-animate' ) && toSlide.hasAttribute( 'data-auto-animate' ) ) {
			...
			dispatchEvent( dom.wrapper, 'autoanimate', { fromSlide: fromSlide, toSlide: toSlide, sheet: autoAnimateStyleSheet } );
				...
				toArray( fragments ).forEach( function( el, i ) {
					...
					if( i <= index ) {
						...
						if( !wasVisible ) {
							...
							dispatchEvent( el, 'visible', null, { bubbles: false } );
						}
					}
					// Hidden fragments
					else {
						...
						if( wasVisible ) {
							...
							dispatchEvent( el, 'hidden', null, { bubbles: false } );
						}
					}
		...
		if( currentSlide && config.fragments ) {
			...
			if( fragments.length ) {
				...
				if( changedFragments.hidden.length ) {
					dispatchEvent( dom.wrapper, 'fragmenthidden', { fragment: changedFragments.hidden[0], fragments: changedFragments.hidden } );
				}

				if( changedFragments.shown.length ) {
					dispatchEvent( dom.wrapper, 'fragmentshown', { fragment: changedFragments.shown[0], fragments: changedFragments.shown } );
				}
	...
	function pauseAutoSlide() {
		...
		if( autoSlide && !autoSlidePaused ) {
			...
			dispatchEvent( dom.wrapper, 'autoslidepaused' );
			clearTimeout( autoSlideTimeout );
	...
	function resumeAutoSlide() {
		...
		if( autoSlide && autoSlidePaused ) {
			autoSlidePaused = false;
			dispatchEvent( dom.wrapper, 'autoslideresumed' );
			cueAutoSlide();
		}
